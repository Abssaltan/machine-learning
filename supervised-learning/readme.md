<h1>Supervised Learning</h1>

<p>
  This directory contains implementations of major 
  <strong>supervised machine learning algorithms</strong> for both 
  classification and regression tasks. Each notebook demonstrates the 
  full workflow: data preprocessing, model training, evaluation, and result visualization.
</p>

<h2>📚 Included Algorithms</h2>

<h3>Classification Algorithms</h3>
<ul>
  <li><strong>ID3.ipynb</strong> – Decision tree using information gain (entropy)</li>
  <li><strong>c4.5.ipynb</strong> – Improved decision tree using gain ratio</li>
  <li><strong>decisiontree.ipynb</strong> – Generic decision tree classifier</li>
  <li><strong>knn.ipynb</strong> – K-Nearest Neighbors classifier</li>
  <li><strong>naivebayes.ipynb</strong> – Naive Bayes classifier</li>
  <li><strong>logisticregression.ipynb</strong> – Logistic Regression (binary/multi-class)</li>
  <li><strong>supportvector.ipynb</strong> – Support Vector Machine (SVM)</li>
  <li><strong>svc.ipynb</strong> – Support Vector Classifier using scikit-learn</li>
  <li><strong>Randomforest.ipynb</strong> – Random Forest ensemble model</li>
  <li><strong>Stochastic_Gradient_Descent.ipynb</strong> – SGD Classifier for large-scale data</li>
</ul>

<h3>Regression Algorithms</h3>
<ul>
  <li><strong>linear_regression.ipynb</strong> – Simple / multiple linear regression</li>
  <li><strong>Polynomial_regression.ipynb</strong> – Polynomial curve-fitting regression</li>
</ul>

<h2>📁 Purpose of This Folder</h2>
<p>
  This collection serves as a reference and learning resource for understanding 
  core supervised ML models. It is ideal for:
</p>
<ul>
  <li>Practicing ML algorithms</li>
  <li>Academic and assignment-based work</li>
  <li>Comparing algorithm performances</li>
  <li>Interview and concept preparation</li>
</ul>

<h2>🧩 How to Use These Notebooks</h2>

<p><strong>1. Clone the repository</strong></p>
<pre>
git clone https://github.com/Abssaltan/machine-learning.git
cd machine-learning/supervised-learning
</pre>

<p><strong>2. Install dependencies</strong></p>
<pre>
pip install numpy pandas matplotlib scikit-learn
</pre>

<p><strong>3. Run a notebook</strong></p>
<pre>
jupyter notebook filename.ipynb
</pre>

<h2>📈 What You Will Learn</h2>
<ul>
  <li>How supervised learning algorithms work internally</li>
  <li>Differences between classification and regression models</li>
  <li>How to evaluate model performance using common metrics</li>
  <li>How to visualize model accuracy and predictions</li>
</ul>

<h2>✅ Future Enhancements</h2>
<ul>
  <li>Cross-validation implementations</li>
  <li>Regularized regression (Lasso, Ridge)</li>
  <li>Gradient Boosting & XGBoost</li>
  <li>Hyperparameter tuning</li>
</ul>

<h2>📄 License</h2>
<p>
  This repository is for educational and learning purposes. Feel free to explore and adapt the notebooks.
</p>

